...
[EDIT]  [[https://lambdaisland.com/episodes/clojure-core-some#/transcript][SOURCE: Lambda Island Some Video]] found out that the idiom at least for ~apply or~ is ~some identity coll~ This is brilliant! However it's late and I'll have to come back and see if there is an equivalent idiom for ~apply and~ which I suspect is something maybe to do with ~remove~.

So the basic idea behind using ~some indentity coll~ is that some acts as if it is an ~or~ statement. Let's think about this and what it would look like for and:
#+begin_src clojure
;; so we have this similarity between or and some
(or (even? 5)
    (even? 7)
    (even? 8))
;; => true

;; if we have this same series of numbers as a collection we can use some
(some even? [5 7 8])
;; => true

;; so is there something here that can be used with and since ~and~ and ~or~ are
;; macros we cannot use them as ~apply and~ or ~apply or~, in other words what
;; if we want to ~and~ an entire seq?

;; so ~or~ returns the first truthy value ~and~ returns the first falsy value
;; so when we are or-ing across a collection we are asking if anything here is
;; truthy, conversely and-ing through a collection must mean we are asking are any
;; of these values falsey?
...
;; it's funny to me after years of programming I'm having to develop a feel for
;; simple boolean logic but something is missing in my mental model so here goes:
...
;; so one metaphor that comes to mind is that ~and~ is like a null hypothesis
;; in that you are trying to prove something is false while taking that it is
;; true throughout the test (innocent until proven guilty -- which to me is a valid explanation of why (and) => true), and if at the end of
;; your test you have not found a "contradiction" then the assertion is true
;; (or you did not have enough data) so ~and~ is looking for contradictions and
;; or is looking for validations is how I'm seeing it.

;; I suppose one of the main difficulties for me is realizing that ~and~ and ~or~
;; return the value when they evaluate to true or false instead of returning
;; true or false, which by now seems incredibly powerful!

;; so coming back to our attempt to figure out a way to use ~apply and~ and what
;; that would mean we have the following discovery:

;; looking into clojure.core we can see that not-any? is defined as:
;; (def not-any? (comp not some)) which is neat, and the direct opposite of
;; some but is assuming that ~or~ is the direct opposite of ~and~ and is that true?
;; My first inclination is to suspect that ~or~ and ~and~ are not opposites because
;; the opposite of (and) ;=> true does not equal (or) ;=> nil

(not-any? even? [5 7 8])
;; => false

;; so while not-any? even? coll returns the opposite of some even? coll
;; this absolutely would not be the equivalent to ~apply and~ which if we could
;; just find opposite of some (which is not-any?) and cut and paste it into the
;; previous idiom:

(some identity [nil nil 1 3]);; => 1

;; which does seem to produce the same results as ~apply or~ but not-any? does not

(not-any? identity [nil nil 1 3]);; => false

;; if the above were ~apply and~ the result would be nil hmmmm
;; it seems again that my previous conception of ~and~ and ~or~ are booleans
;; not-any? and not-every? are both booleans and so they cannot return a
;; value

;; i suppose I will have to ask if there is such an idiom as ~apply and~
;; well maybe not...and the logic of wanting a return value when you are looking
;; for some is that there are many versions of truthiness and only two versions
;; of nil, so it may not be so interesting to know if the false value is nil or
;; false literal but rather that the collection contains at least one of those
;; literal values where as when looking at truthiness it's more valuable to
;; get the value back since MOST values are truthy we most likely want to do
;; something with the value and if a collection of values is false this can
;; tell us something more concrete about the entire collection perhaps and
;; if we want to investigate the specifics of that falsey value we have specific
;; functions for that nil? and false?
#+end_src
...
** [[https://www.4clojure.com/problem/95][#95 To Tree or Not to Tree]]
   Write a predicate which checks whether or not a given sequence represents a binary tree. Each node in the tree must have a left child and a right child. Running through a visual check of the tests shows that one tricky part of this is that each node must have either two children or none which caught me out for a brief moment. I believe this is called a full tree. We can start with a naive assumption that the total count of a sequence must be 3 to satisfy the binary-ness of the problem and also make sure that if there is a nil that both children are nil could tree-seq work?
#+begin_src clojure
(def tt [1 [2 [3 [4 false nil] nil] nil] nil])

(tree-seq sequential? identity tt)
;; => (
;;[1 [2 [3 [4 false nil] nil] nil] nil]
;;1
;;[2 [3 [4 false nil] nil] nil]
;;2
;;[3 [4 false nil] nil]
;;3
;;[4 false nil]
...
;; so I'm a little blocked at this point, I'm wondering if thinking of the
;; tree structure is making this too opaque, like I don't have the proper
;; mental models to fluently navigate the necessary steps, perhaps it's better
;; to approach with a basic recursive approach building up a collection of
;; tuples that hold true/false and the count and the count of the solution and
;; pass that collection through a predicate...the problem statement is to
;; write just a predicate though, which is just a function that returns t/f
;; right? Is there a simple predicate that doesn't necessarily transform
;; the collection but still checks it's binary tree-ness?

(count tt);; => 3
;; ok there's a clue that I'm making this too difficult, we can count at all
;; levels I believe with tree-seq

;; revisiting tree-seq shows that it returns a lazy-seq of the nodes in a
;; tree, via a depth first walk, meaning it descends if it has the chance
;; it takes predicate branch? which examines and determines whether there are
;; children that need to be included in the processing and children fn takes a
;; single argument that selects or generates the children that are returned

;; the tree structure is important if I am to consider that I need to check
;; that both children are nil or the tree is not full, this I assume requires
;; some semblance of the tree structure (I cannot just flatten and count
;; to see if the number divides evenly by 3 and does not contain false)
...
(tree-seq #(and (sequential? %) (= 3 (count %))) rest tt)
;; => (
;; [1 [2 [3 [4 false nil] nil] nil] nil]
;; [2 [3 [4 false nil] nil] nil]
;; [3 [4 false nil] nil]
...
;; gotta come back to #95 TODO
#+end_src
   TODO
** [[https://www.4clojure.com/problem/96][#96 Beauty is Symmetry (binary trees)]]
   TODO
...
;; => (8 9 1)
#+end_src
** [[https://www.4clojure.com/problem/100][#100 Least Common Multiple]]
   TODO
...
** [[https://www.4clojure.com/problem/118][#118 Reimplement Map]]
   Given a function f and an input sequence s, return a lazy sequence of (f x) for each element x in s: first thought, cons recursion
#+begin_src clojure
((fn m [f s]
   (when (not-empty s)
     (cons (f (first s)) (m f (rest s))))) inc [2 3])

;; this was fairly easy for me but most likely because by now I have been
;; exposed to the map function for a good while. What was less clear is the
;; discussion of
;; a) what function to use that works on a collection and returns nil vs '()
;; (next ());; => nil and (rest ());; => ()
;; conversely taking from the front we have (first ());; => nil and
;; (take 1 ());; => ()
;; these differences really matter with recursion since the last step in cons
;; needs to finish with an empty seq (not nil)
;; and b) how to determine emptiness of a collection, using
;; (not (empty? '())) ;; => false is frowned upon without explanation although
;; some sources site redundancy in the macro expansion and performance and instead
;; the recommendation is to use (seq '());; => nil which does not feel as readable
;; because one has to simply remember that calling seq on an empty coll is nil
;; which doesn't feel "emergent"
#+end_src

** [[https://www.4clojure.com/problem/120][#120 Sum of Square of digits]]
   Write a function which takes a collection of integers as an argument and returns the count of how many elements are smaller than the sum of their squared component digits, so take a number, convert to string, filter to build up a list and return the count of that sequence:
#+begin_src clojure
((fn [c]
   (count
     (filter #(< (first %) (second %))
                        ...
                        (interleave c
                                    (map
                                      (fn [x] (reduce
                                                ...
                                                (str x))) c)))))) (range 30))
;; => 19
;; fighting through some kind of brain fog here, not the clearest work
;; and feels like I've chiseled a solution out of rock, hard work!

;; including a solution that is very similar but highlights that
;; let can be used to define functions that apply within the context of
;; the parent function. Obvious after having seen it firsthand.
...
;; the function body for digits: (fn [n] (map #(- (int %) 48) (str n)))
;; simply takes the char-digits and converts them to ints. Here I like
;; my version better because it uses \0 and avoids the "what kind the magic
;; number 48 is doing"

;; square squares the number

;; sum digits applies reduce + (which I always just see as apply +) to the
;; square of all the numbers.

;; finally we return the count as a result of the composition of all of the above
;; with a filter to check if the number in question is less than the sum
;; of the square of the digits

;; thank goodness for let, it makes my solution look like a spaghetti monster!
#+end_src
...
#+end_src
** [[https://www.4clojure.com/problem/128][#128 Recognize Playing Cards]]
   Cards are 0 (the 2) to 12 (the ace) and a string like "SJ" should be parsed into {:suit :spade :rank 9}. Use "T" instead of "10" so that all strings are only 2 chars.
#+begin_src clojure
;; so this seems like a matrix with suit as the columns and 0-12 as the rows
;; so all the cards would look something like

(let [cards (for [suit [:spade :heart :diamond :club]
                  rank (into (into [] (range 2 10)) ["T" "J" "Q" "K" "A"])]
              [suit rank])]
  cards)
...
(zipmap (map (comp keyword str)
             (filter #(not= \space %) (into (into [] (apply str (range 2 10))) ["T" "J" "Q" "K" "A"])))
        (range 0 13))

(zipmap [:S :H :D :C] [:spade :heart :diamond :club])

(let [[a b] "SJ"] (str a " " b))
;; => "S J"
;; so we would want "2" -> 0 and "A" -> 12
;; and "S" -> :spade

;; so if we had two maps one with keyword to suit and keyword to rank
;; that would be the problem solved, somehow I still want to be fancy tho

((fn [[s r]]
   (let [suit (zipmap [:S :H :D :C] [:spade :heart :diamond :club])
         rank (zipmap (map (comp keyword str)
                           (filter #(not= \space %) (into (into [] (apply str (range 2 10))) ["T" "J" "Q" "K" "A"])))
                      (range 0 13))]
     (into {:suit ((keyword (str s)) suit)} {:rank ((keyword (str r)) rank)}))) "SJ")
#+end_src
I was surprised to find that of the solutions I looked at none seemed to stray far from my version of the answer...curious I was expecting something more sophisticated but alas, simple works!
#+begin_src clojure
;; so the first is similar to my solution except that instead of using
;; keywords the answer simple uses strings as keys, which simplifies the code

(fn card [[s r]] ; similar destructuring takes suit and rank from strings
  ;; suits and ranks also use strings instead of vectors of strings which
  ;; notably complicated my solution
  (let [suits (zipmap (map str "SHCD") [:spade :heart :club :diamond])
        ranks (zipmap (map str (concat (range 2 10) "TJQKA")) (range 13))]
    {:suit (suits (str s)) :rank (ranks (str r))} ; and there's no need to use into
                                        ; when it's simple enough to build up a hashmap directly
    ))

;; one last solution demonstrates my initial thinking of getting the index of
;; a matching key
...
;; I have to imagine that this is a good balance of short and still readable
;; here the answer uses chars as keys which is legal and direct
;; using the java interop for indexOf demonstrates the power of knowing a little
;; bit of java, which is one of my many weak points

;; all in all I find this to be the most direct solution, no need for intermediate
;; variables and building the map directly using the built-in map behaviors native
;; to clojure --> is this what [[https://lispcast.com/what-is-point-free-style/][Point-Free Style]] looks like?
#+end_src
...
** [[https://www.4clojure.com/problem/146][#146 Trees into Tables]]
   TODO
** [[https://www.4clojure.com/problem/147][#147 Pascal's Trapezoid]]
   TODO
   TODO
** [[https://www.4clojure.com/problem/153][#153 Pairwise Disjoint Sets]]
   Ok got it, set of sets, see if any of the internal sets have any overlapping elements or return true if each set is disjointed
#+begin_src clojure
(def test1 #{#{\U} #{\s} #{\e \R \E} #{\P \L} #{\.}})
(def test2 #{#{:a :b :c :d :e}
             #{:a :b :c :d}
             #{:a :b :c}
             #{:a :b}
