...
;; [spoiler alert, nothing special is happening because nesting isn't happening]
...
;; then reduce happens with the same reduce logic however the intermediate outputs
